...
import time
from utility.get_data import UcfData
...
class MultiTrainer(object):
    ...
    def train(self):
        ...
        with tf.Graph().as_default():
            ...
            tower_grads = []
            logits = []
            learning_rate = tf.train.exponential_decay(cfg.learning_rate, global_step, cfg.decay_steps, cfg.decay_rate,
                                                       staircase=True)
            ...
            with tf.variable_scope(tf.get_variable_scope()):
                for gpu_index in range(0, self.num_gpus):
                    with tf.device('/gpu:%d' % gpu_index):
                        loss, logit = self.tower_loss(
                            history_placeholder[gpu_index * self.batch_size:(gpu_index + 1) * self.batch_size, :],
